...
BEGIN
  ...
  IF NEW.movement_type = 'INVOICE_VOID_RETURN'::public.stock_movement_type THEN
    -- Cari semua batch yang dipakai untuk produk ini di SALE_OUT movement dari invoice yang sama
    FOR batch_row IN
      SELECT DISTINCT sa.batch_id, SUM(sa.quantity) as total_allocated
