...
interface ReceiptRow { id: string; productId: string; manualQuantity: string; digitalQuantity: string; unitCost: number; }
...
export function AddStockReceiptDialog({ products, suppliers }: AddStockReceiptDialogProps) {
  ...
  const addItem = () => setItems((current) => [...current, { id: `receipt_${Date.now()}_${current.length}`, productId: "", manualQuantity: "1", digitalQuantity: "1", unitCost: 0 }]);
  ...
  const total = items.reduce((sum, item) => sum + (Number(item.manualQuantity) || 0) * item.unitCost, 0);
  const totalManualQuantity = items.reduce((sum, item) => sum + (Number(item.manualQuantity) || 0), 0);
  const totalDigitalQuantity = items.reduce((sum, item) => sum + (Number(item.digitalQuantity) || 0), 0);
  const totalWeightDifference = totalDigitalQuantity - totalManualQuantity;
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await createStockReceiptAction({
      ...
      supplierReference: supplierReference || undefined, createPayable, notes: notes || undefined,
       items: items.map((item) => ({ productId: item.productId, manualQuantity: Number(item.manualQuantity), digitalQuantity: Number(item.digitalQuantity), unitCost: item.unitCost })),
    });
      ...
      <DialogContent className="max-h-[min(92dvh,820px)] max-w-[calc(100%-1.5rem)] overflow-y-auto rounded-2xl sm:max-w-3xl">
        ...
        <form onSubmit={async (event) => { setSaving(true); await submit(event); setSaving(false); }} className="space-y-5">
          ...
          <div className="overflow-hidden rounded-xl border border-stone-200">
             <div className="flex items-center justify-between gap-3 border-b border-stone-200 bg-stone-50 px-3 py-3"><div><p className="text-sm font-semibold text-stone-800">Produk diterima</p><p className="text-xs text-stone-500">Berat manual dipakai untuk pembayaran; berat digital dipakai untuk menambah stok.</p></div><Button type="button" size="sm" variant="outline" onClick={addItem} className="h-10 rounded-xl px-3.5"><Plus className="mr-1 h-3.5 w-3.5" /> Tambah</Button></div>
              {items.length === 0 ? <div className="px-4 py-8 text-center text-xs text-stone-500">Belum ada produk. Tambahkan minimal satu produk.</div> : <div className="divide-y divide-stone-100">{items.map((item, index) => { const product = products.find((entry) => entry.id === item.productId); const difference = (Number(item.digitalQuantity) || 0) - (Number(item.manualQuantity) || 0); return <div key={item.id} className="grid gap-3 p-3 sm:grid-cols-[minmax(0,1fr)_7rem_7rem_9rem_auto] sm:items-start"><div className="space-y-1.5"><label className="text-[11px] font-semibold text-stone-600">Produk {index + 1}</label><select required value={item.productId} onChange={(event) => chooseProduct(item.id, event.target.value)} className="h-10 w-full rounded-xl border border-stone-200 bg-white px-3 text-sm outline-none focus:border-stone-500"><option value="">Pilih produk</option>{activeProducts.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}{entry.size ? ` [${entry.size}]` : ""}</option>)}</select>{product && <p className="text-[11px] text-stone-500">Satuan: {product.defaultUnit} · stok saat ini {product.stockQuantity ?? 0}</p>}</div><div className="space-y-1.5"><label className="text-[11px] font-semibold text-stone-600">Timbangan manual</label><Input required min="0.001" step="0.001" type="number" value={item.manualQuantity} onChange={(event) => updateItem(item.id, "manualQuantity", event.target.value)} className="h-10 rounded-xl text-right tabular-nums" /></div><div className="space-y-1.5"><label className="text-[11px] font-semibold text-stone-600">Timbangan digital</label><Input required min="0.001" step="0.001" type="number" value={item.digitalQuantity} onChange={(event) => updateItem(item.id, "digitalQuantity", event.target.value)} className="h-10 rounded-xl text-right tabular-nums" /><p className={`text-[10px] text-right font-medium ${difference < 0 ? "text-red-600" : "text-emerald-700"}`}>Selisih {Math.abs(difference).toFixed(1)} {product?.defaultUnit || "unit"}</p></div><div className="space-y-1.5"><label className="text-[11px] font-semibold text-stone-600">Harga / manual</label><CurrencyInput required min={0.01} value={item.unitCost} onChange={(value) => setItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, unitCost: value } : entry))} placeholder="85000" /></div><Button type="button" variant="ghost" size="icon" onClick={() => removeItem(item.id)} className="h-10 w-full rounded-xl text-stone-400 hover:bg-red-50 hover:text-red-600 sm:mt-6 sm:w-10" aria-label={`Hapus produk ${index + 1}`}><Trash2 className="h-4 w-4" /></Button></div>; })}</div>}
             <div className="flex flex-col gap-3 border-t border-stone-200 bg-stone-50 px-3 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"><div><span className="font-medium text-stone-600">Total pembelian</span><span className="block text-[11px] font-normal text-stone-500">Dihitung dari timbangan manual (berat yang dibayar)</span></div><div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-right"><span className="text-xs text-stone-500">Manual {totalManualQuantity.toFixed(1)} · Digital {totalDigitalQuantity.toFixed(1)} · Selisih <strong className={totalWeightDifference < 0 ? "text-red-600" : "text-emerald-700"}>{Math.abs(totalWeightDifference).toFixed(1)}</strong></span><span className="font-bold tabular-nums text-stone-900">{formatCurrency(total)}</span></div></div>
          </div>

           <div className="rounded-xl border border-sky-200 bg-sky-50/60 p-3.5"><label className="flex items-start gap-2.5 text-sm text-sky-950"><input type="checkbox" checked={createPayable} onChange={(event) => setCreatePayable(event.target.checked)} className="mt-0.5 size-4 accent-sky-700" /><span><span className="font-semibold">Buat hutang supplier dari penerimaan ini</span><span className="mt-0.5 block text-xs text-sky-800/75">Jika dicentang, total pembelian memakai berat manual; stok memakai berat digital.</span></span></label>{createPayable && <div className="mt-3 max-w-xs space-y-1.5"><label htmlFor="stock-receipt-due" className="text-xs font-semibold text-sky-900">Jatuh tempo</label><Input id="stock-receipt-due" type="date" min={receivedDate} value={dueDate} onChange={(event) => setDueDate(event.target.value)} className="h-10 rounded-xl border-sky-200 bg-white" /></div>}</div>
